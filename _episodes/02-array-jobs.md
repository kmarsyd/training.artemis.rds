...
The above **directive** in a **PBS script** will launch 8 copies of the job defined in the script, with each copy having an **array index** equal to an integer from _4 to 25_ in steps of _3_.
...
>
> This script isn't doing much! Leave as is, with **1 minute** of wall-time, and the minimum RAM, **1 GB**, and **1 CPU**.
> ~~~
...
What will this job do? Firstly, note that array job directive initialising 10 copies of this job, with indices 1,2,...,10:
...
Note that for _subjobs_ of an array, complete status is not indicated by '**F**' but '**X**', meaning 'exited'. You can query individual subjobs as well by indexing them
...
This meant that the standard output log files generated by the PBS Scheduler were _empty_, since the output of ```printf``` was sent elsewhere. However, we can customise PBS' log files directly using the subjobs' array indices themselves.
...
And, again, an individual subjob
